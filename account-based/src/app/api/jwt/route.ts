 ...
 * @api {get} /jwt?verificationType=:verificationType
...
export const GET = async (request: NextRequest) => {
  try {
    ...
    const token = jwt.sign(
      {
        iss: process.env.NEXT_PUBLIC_DOMAIN_NAME!, // Obtained from Authento dashboard
        sub: session.user.id, // userIdentifier: identification string unique to each user
      ...
      process.env.AUTHENTO_JWT_SECRET! // Obtained from Authento dashboard
    );
